...
Berkas terpisah: **[UPGRADE.md](UPGRADE.md)**.

Halaman ini menjelaskan pemasangan dari nol. Menaikkan versi server yang
sudah melayani punya bahaya yang sama sekali lain — data absensi dan wajah
terdaftar bisa hilang bila langkahnya dibalik — jadi urutannya ditulis
sendiri di sana, bukan disisipkan sebagai satu subbagian di sini.

Dua hal yang paling mudah menjatuhkan pembaruan, keduanya dibahas di berkas
itu: migrasi `0014` berhenti dengan sengaja bila `face_embeddings` masih
berisi vektor 512-d, dan `.env` di server **tidak** ikut terbarui oleh
`git pull`.
