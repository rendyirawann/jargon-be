...
## 2.13 Memperbarui server yang SUDAH berjalan

Bagian ini untuk pemasangan yang sudah hidup dan ingin dinaikkan ke versi
terbaru. Bukan pemasangan baru — untuk itu lihat §2.7.

> **BACA DULU SEBELUM `git pull`.** Pembaruan ini mengubah dimensi embedding
> wajah dari 512 menjadi 128. Bila server Anda sudah punya wajah terdaftar,
> migrasi `0014` akan **berhenti dengan sengaja** dan tidak merusak apa pun —
> tetapi Anda harus memutuskan lebih dulu apa yang dilakukan atas data itu.
...
* **0 baris** — aman, lanjut ke langkah 2.
* **ada baris** — pilih satu:
  * **Daftarkan ulang.** Kosongkan tabelnya secara sadar, lalu daftarkan
    ulang lewat `/admin/biometric`. Embedding 512-d dan 128-d tidak bisa
    dikonversi: 128 angka itu bukan ringkasan dari 512 angka yang lain,
    melainkan keluaran model yang sama sekali berbeda.
    ```bash
    docker compose exec -T postgres psql -U absensi -d absensi -c \
  ...
  * **Tetap di 512-d.** Jangan tarik pembaruan ini. Pengenalan wajah lewat
    browser memang belum bisa dipakai, tetapi jalur tablet tetap utuh.

### Langkah 2 — cadangkan

Sekali perintah, sebelum apa pun berubah. Migrasi `0014` mengubah tipe
kolom, dan sqlx tidak punya migrasi turun.
...
`0.82`, bukan `0.62`. Ambang itu bergantung model — alasannya di §2.12.
Membiarkan `0.62` berarti wajah orang lain bisa tercatat sebagai siswa.
...
Tiga migrasi yang baru: `0014` (vektor 128-d), `0015` (izin
`operate_face_kiosk`), `0016` (penajaman index).
...
Yang diharapkan: migrasi teratas `16 index optimasi`, dan konfigurasi
terbaca `128 faceapi-v1 0.82`.

Lalu buka `/admin/biometric/scan`, pasangkan perangkat dengan kode dari
`/admin/devices`, dan uji satu wajah sampai tercatat.
